...
This repository solves Exercism exercises in Gleam, with each exercise living under `gleam/` as its own Gleam project.
...
| --- | --- |
| `just install` | Prints the installed Gleam version, confirming the toolchain is available |
| `just format` | Formats every Gleam exercise project under `gleam/` |
| `just lint` | Checks that every Gleam exercise project is formatted, without modifying files |
| `just test` | Runs the test suite for every Gleam exercise project under `gleam/` |
| `just test-exercise EXERCISE` | Runs the test suite for a single exercise directory under `gleam/` |
| `just check` | Runs `lint` then `test`, the full pre-pull-request check |
| `just update` | Updates Gleam dependencies for every exercise project |

1. Fork the repository and clone your fork locally
2. Create a branch for your change (see [Branches](#branches) for naming)
3. Run `just check` to format-check and test before opening a pull request
